...
void twistMessageReceived(const geometry_msgs::Twist& msg)
{
  ...
  set_Lvel = set_lin + (set_ang * WHEEL_SEPARATION / 2); //target_speed_l
  set_Rvel = set_lin - (set_ang * WHEEL_SEPARATION / 2); //target_speed_r
  set_LRPM = set_Lvel * 60 / 2 / PI / WHEEL_RADIUS; //RAW to RPM
  set_RRPM = set_Rvel * 60 / 2 / PI / WHEEL_RADIUS; //RAW to RPM
...
void position_tracking()
{
  ...
  L_Vel = 2 * PI * WHEEL_RADIUS * L_RPM / 60; //left wheel velocity
  R_Vel = 2 * PI * WHEEL_RADIUS * R_RPM / 60; //right wheel velocity
...
{  
  ...
  double siny_cosp = 2 * (x[0] * x[3] + x[1] * x[2]);
  double cosy_cosp = 1 - 2 * (x[2] * x[2] + x[3] * x[3]);
